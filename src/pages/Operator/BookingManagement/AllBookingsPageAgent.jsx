...
import { getAllBookingsForOperatorApiCall, updateBookingStatusApiCall } from '../../../services/OperationsService';
import { format } from 'date-fns';

const AllBookingsPageAgent = () => {
  const [page, setPage] = useState(1);
  const [refetchIndex, setRefetchIndex] = useState(0);
  const forceRefetch = () => setRefetchIndex(prev => prev + 1);

  const { data: response, isLoading, error } = useFetch(
    () => getAllBookingsForOperatorApiCall(page),
    [page, refetchIndex]
  );
...
export default AllBookingsPageAgent;
